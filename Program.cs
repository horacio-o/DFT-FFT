...
namespace DFT_a_FFT
{
    internal class Program
    {
        ...
        {
            //System.Media.SoundPlayer player = new System.Media.SoundPlayer(@"C:\Users\Tonda\Seminarka\Oof.wav");
            //player.Play();


            //using (FileStream stream = File.OpenRead(@"C:\Users\Tonda\Downloads\Oof.wav")) //test
            //{
            //    BinaryReader reader = new BinaryReader(stream);
            //    Console.WriteLine(string.Join(" ", WavChunkLookUp(reader).Keys)); //na pokus jsou .Values nebo .Keys
            //    Console.ReadLine();
            //}

            //using (FileStream stream = File.OpenRead(@"C:\Users\Tonda\Seminarka\Oof.wav"))
            //{
            //    BinaryReader reader = new BinaryReader(stream);
            //    Console.WriteLine(string.Join(" ", InspectWav(reader).samples));
            //    Console.ReadLine();
            //}
            
            using (FileStream stream = File.OpenRead(Console.ReadLine()))
            {
                BinaryReader reader = new BinaryReader(stream);
                SampleArray sampleArray = InspectWav(reader);
                FrequencyData[] fourie = DFT(sampleArray.samples, sampleArray.sampleRate);
                Console.WriteLine(string.Join(" ", fourie));
                Console.ReadLine();
            }
        }
        
        //public static Complex[] DiscoTransform(Complex[] input)
        //{
